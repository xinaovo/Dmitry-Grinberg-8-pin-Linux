...
#if defined(FPU_SUPPORT_FULL) || defined(FPU_SUPPORT_MEDIUM)
	#define FPU_REVISION		0x320
#elif defined(FPU_SUPPORT_MINIMAL)
	#define FPU_REVISION		0
#else
	#error "building fpu with no fpu on"
#endif
...
enum FpuOpRet fpuOp(uint32_t instr, uint32_t *cpuRegs, struct FpuState *fpu)
{
	...
	switch ((instr >> 21) & 0x1f) {
		...
		goto inval;	//mark as op we could not perform
	#endif

	#ifdef FPU_SUPPORT_MEDIUM	//we support simple things
